...
export default function NounTranslationPractice() {
  ...
  const generateOptions = useCallback((correctAnswer: string, allNouns: NounData[]): string[] => {
    ...
    while (optionsSet.size < 4) {
      ...
      const word = allNouns[randomIndex].germanWord;
      if (word && word !== correctAnswer) optionsSet.add(word);
    }
  ...
  const selectRandomNoun = useCallback(() => {
    ...
    setUsedIndices(prevUsed => {
      ...
      setOptions(generateOptions(nouns[selectedIndex].germanWord, nouns));
  ...
  const handleAnswer = (selectedGermanWord: string) => {
    ...
    if (selectedGermanWord === currentNoun.germanWord) {
      ...
      setToast({ type: 'correct', message: `Correct! "${currentNoun.noun}" is "${currentNoun.germanWord}".` });
    } else {
      setToast({ type: 'incorrect', message: `"${currentNoun.noun}" is "${currentNoun.germanWord}".` });
    }
  };
  ...
  if (!currentNoun || options.length === 0) return <div className="practice-container"><div className="loading">Preparing practice...</div></div>;
        ...
        <div className="card-content">
          ...
          <div className="options">
            {options.map((germanWord) => (
              <button
                key={germanWord}
                className={`option-button ${isAnswered ? (germanWord === currentNoun.germanWord ? 'correct' : 'incorrect') : ''}`}
